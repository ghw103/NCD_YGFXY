...
ScanCodeResult ScanCodeFun(QRCode * parm)
{
	...
	while(pdPASS == ReceiveDataFromQueue(GetUsart2RXQueue(), GetUsart2RXMutex(), readQRCodeBuffer->originalcode , MAX_QR_CODE_LENGHT, 1, 50 * portTICK_RATE_MS, 100 / portTICK_RATE_MS))
		;
...
static void ReadBasicCodeData(ReadQRCodeBuffer * readQRCodeBuffer)
{
	...
	while(pdPASS == ReceiveDataFromQueue(GetUsart2RXQueue(), GetUsart2RXMutex(), readQRCodeBuffer->originalcode , MAX_QR_CODE_LENGHT, 1, 10 / portTICK_RATE_MS, 10 / portTICK_RATE_MS))	
		;
